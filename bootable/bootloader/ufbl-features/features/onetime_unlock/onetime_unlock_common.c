...
static int amzn_verify_onetime_unlock_cert(const onetime_unlock_cert_t *cert, unsigned int cert_len)
{
    ...
    ret = amzn_crypto_impl_sha256(cert->pubkey, cert->pk_len, digest);
    if (ret) {
...
{
    return amzn_crypto_impl_b64_encode(in, len, out, outlen);
}
...
{
    return amzn_crypto_impl_b64_decode(b64_str, len_in, b64_dec, len_out);
}
