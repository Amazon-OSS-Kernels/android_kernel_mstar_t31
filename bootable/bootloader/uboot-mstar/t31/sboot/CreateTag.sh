...
CherryPick_SBoot:$CHERRYTMP"
	...
	echo CherryPick_tv_tee_loader:[Enter to continue]
	read CHERRYTMP
	if [ "$CHERRYTMP" ];then
		TAGMESSAGE+="
CherryPick_tv_tee_loader:$CHERRYTMP"
	fi
}
...
if [ -d "../tv_tee_loader/" ];then
	cd ../tv_tee_loader/
	tv_tee_loader_Commit=$(git rev-parse HEAD)
fi
...
if [ -d "../tv_tee_loader/" ]; then
	TAGMESSAGE+="
TvTeeloader: $tv_tee_loader_Commit"
fi
TAGMESSAGE+="
