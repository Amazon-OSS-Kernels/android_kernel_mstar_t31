...
#include "../../../MstarApp/include/MsTrustZone.h"
...
#if defined(CONFIG_MSTAR_M5621) || defined(CONFIG_MSTAR_M7322) || defined(CONFIG_MSTAR_MAINZ) || defined(CONFIG_MSTAR_MASERATI) || defined(CONFIG_MSTAR_M7221) || defined(CONFIG_MSTAR_M7642) || defined(CONFIG_MSTAR_M7621)
...
#define FLAG_ANTIROLLBACK_INITIALIZED (0x42525458)
...
#define CHIP_GLOBAL_RESET() do { ns_uboot_DelayMS(3000); *(volatile unsigned short*)(0x1F005CB8) = 0x79; } while(0)
...
static unsigned int _gUbootVersion = 0;
static unsigned int * _gUbootVersionAddr = NULL;
static const unsigned int _gSbootVersion = 0x1234;
...
unsigned int ns_uboot_anti_rollback(void)
{
    ...
    if (rpmb->anti_rollback_init_flag == FLAG_ANTIROLLBACK_INITIALIZED &&
       (rpmb->teeloader_version > VERSION_NUMBER_TEELOADER || rpmb->optee_version > _gOpteeVersion || rpmb->armfw_version > _gArmFwVersion || rpmb->teekeybank_version > _gTEEKeybank.u32TkbVersion || rpmb->hash1_version > _gHash1Version) )
    {
        LDR_PUTS("~");
        authFailType = AUTH_OPTEE_FAIL;
        return FALSE;
             ...
             rpmb->teeloader_version < VERSION_NUMBER_TEELOADER || rpmb->optee_version < _gOpteeVersion || rpmb->armfw_version < _gArmFwVersion || rpmb->teekeybank_version < _gTEEKeybank.u32TkbVersion || rpmb->hash1_version < _gHash1Version)
    {
        rpmb->hash1_version = _gHash1Version;
        rpmb->teekeybank_version = _gTEEKeybank.u32TkbVersion;
        rpmb->teeloader_version = VERSION_NUMBER_TEELOADER;
...
unsigned int ns_uboot_load_optee(void)
{
    ...
    if(_gGroupSecureMemFlag)//Use secure group memory as secure memory
    {
        ...
        if (!MDrv_SEAL_SetSecureRange(__BA2PA(_gGroupSecureMemAddr_lo), __BA2PA(_gGroupSecureMemAddr_lo+_gGroupSecureMemsize), SEAL_ATTR_SECURE_RW))
        {
    ...
    {
        if(!_gGroupSecureMemFlag && !MDrv_SEAL_SetSecureRange(__BA2PA(header->init_load_addr_lo), __BA2PA(header->secure_range_end), SEAL_ATTR_SECURE_RW))
        {
            authFailType = AUTH_OPTEE_FAIL;
            return FALSE;
        }
...
unsigned int ns_uboot_load_armfw(void)
{
    ...
    if ( _gIsStr && _gStrMode )
    {
        ...
        if ( _gAuthMode == AESCBC_MAC )
        {
            ...
            if(!MDrv_AESDMA_STR_CBCMAC(__BA2PA(tbl->ATFBase), tbl->ATFSize, __BA2PA(u8ATF_MAC)))
            {
                authFailType = AUTH_ARMFW_FAIL;
                return FALSE;
            }
...
unsigned int ns_uboot_read_keybank(unsigned int keybank_rom_offset)
{
    ...
    // Auth TEEKeybank
    if (FALSE == MDrv_AESDMA_SecureMain_v2((U8*)((TEE_KEY_BANK*)(tmpBuf))->u8MSID, sizeof(TEE_KEY_BANK)-sizeof(_SUB_SECURE_INFO), (U8*)((TEE_KEY_BANK*)(tmpBuf))->SecureInfo.u8Signature, EmbeddedTeeRsaPub))
    {
...
void ns_uboot_DelayMS(unsigned int msDelay)
{
    ...
    u64Interval = 12000 * msDelay; // clock is running @ 12MHz
