...
int HAL_SEAL_Init(void)
{
    ...
    // DSCMB3 RIU in TEE
    (RIU[SEAL_TZPC_NONPM_BASE + (0x18<<1)]) &= ~(0x0200);

    // IOMMU RIU in TEE h25[10]:global bank
    // IOMMU RIU in TEE h25[15:11] h26[1:0]:local bank
    (RIU[SEAL_TZPC_NONPM_BASE + (0x25<<1)]) &= ~(0xFC00);
    // IOMMU RIU in TEE h26[3:2]:acpu bank
    // IOMMU RIU in TEE h26[8:7]:gpu bank
    (RIU[SEAL_TZPC_NONPM_BASE + (0x26<<1)]) &= ~(0x018F);
    // Allow REE to read h51[12:1]
    (RIU[SEAL_TZPC_NONPM_BASE + (0x51<<1)]) |= 0x1FFE;
    ...
    //M7332 cannot read permission table, therefore we use delay to make sure that data is written.
...
int HAL_SEAL_SetPermissionForSecureRange(unsigned long long startAddr, unsigned long long endAddr, EN_SEAL_SW_AID enAid, EN_SEAL_RANGE_PERMISSION enPermission)
{
    ...
    unsigned int RegID;
    unsigned long long s_permission = 0x0000FFFFFFFF0300ULL;
