...
#define __delay() {while(*(volatile unsigned long long*)(CONFIG_MIU0_BUSADDR) < flag_dummyread);}
...
#define SEAL_NON_PM_GROUP8 \
    ...
    E_SEAL_HSE_PROTECTED_PROT_NONPM, \
...
const eSeal_IP NonPmIpTbl[SEAL_NONPM_TBL_IP_NUM] =
{
    ...
    SEAL_NON_PM_GROUP9, SEAL_NON_PM_GROUP10
};
...
int HAL_SEAL_Init(void)
{
    ...
    // Only Seucre ARiM can modify it - 9'd15
    ...
    for (i=0, offset=0; i<SEAL_NONPM_TBL_IP_NUM; ++i, offset+=0x20)
    {
        if (i == 3 //tzpc_aesdma_protected_miu_ns
            ...
            || i == 125 //E_SEAL_SECU_R2_D_PROT_NONPM
            || i == 126 //E_SEAL_SECU_R2_I_PROT_NONPM
