...
#endif
static gfp_t high_order_iommu_cma_gfp_flags = (((GFP_HIGHUSER | __GFP_NORETRY | __GFP_NOWARN) & (~__GFP_RECLAIM)) | __GFP_COMP & (~__GFP_HIGHMEM)) | GFP_DMA;
static gfp_t low_order_iommu_cma_gfp_flags = (GFP_HIGHUSER | __GFP_COMP & (~__GFP_HIGHMEM)) | GFP_DMA;
														...
														unsigned long flags)
{
	...
	while(len)
	{
		...
		page = dma_alloc_from_contiguous(dev, size>>PAGE_SHIFT, get_order(IOMMU_DISCRETE_CMA_ALIGN));
		if (!page) {
#ifdef CONFIG_MP_MMA_UMA_WITH_NARROW
			if((mma_dma_zone_size <= 0) || (mma_dma_zone_size > 0 && ((phys_addr_t)__pfn_to_phys(mstar_cma->cma->base_pfn)) < (mma_dma_zone_size + memblock_start_of_DRAM())))
				break;
#endif
			printk("%s::%s::%d, dma_alloc_from_contiguous fail!\n", __FILE__, __FUNCTION__, __LINE__);
			goto err1;
		}
		struct cma_cache_node *nnode;
		nnode = kmalloc(sizeof(struct cma_cache_node), GFP_KERNEL);
	...
	while(len){
		...
		for(i = 0; i < NUM_ORDER_IOMMU_CMA; i++){
			...
			if(orders_iommu_cma[i] > 0)
				page = alloc_pages(high_order_iommu_cma_gfp_flags, orders_iommu_cma[i]);
			else
				page = alloc_pages(low_order_iommu_cma_gfp_flags, orders_iommu_cma[i]);
