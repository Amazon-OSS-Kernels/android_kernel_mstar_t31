...
void parameter_radar_dynamic_update(struct parameter_radar_dynamic *para, u16 interval)
{
	...
	para->beta = (interval == INTERVAL_HIGH) ? 154 : 205;
...
void presence_detection(struct presence_handle *handle_2D, struct parameter_app *para, struct parameter_radar_dynamic *para_radar, u8 peak_level)
{
	/* Input:
	...
	*/
	...
	/*reset the none presence counter when there is valid measurementt*/
	handle_2D->no_presence_counter = 0;
	...
	if (handle_2D->frame_counter > (para->turnOn_delay) + 1)
	{
		if ((handle_2D->frame_counter <= (para->trigger_outdate_counter) + 1) && (handle_2D->counter_score > (para->score_ratio) / 100*(handle_2D->frame_counter - 1)))
		{
			handle_2D->status = 1;
		}
		handle_2D->counter_score = 0;
