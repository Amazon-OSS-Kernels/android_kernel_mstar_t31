...
#ifdef CONFIG_MP_ION_PATCH_FAKE_MEM
	if ( (!size || !memblock_is_region_reserved(base, size)) && !fakemem )
#else
			...
			bool fixed, struct cma **res_cma)
{
	...
	int ret = 0;
#ifdef CONFIG_MP_ION_PATCH_FAKE_MEM
	bool fakemem = !pfn_valid(base >> PAGE_SHIFT) && fixed;
#endif
...
#ifdef CONFIG_MP_ION_PATCH_FAKE_MEM
pass_reserve:
	ret = cma_init_reserved_mem(base, size, order_per_bit, res_cma, fakemem);
#else
	...
	if (ret)
		goto err;
#ifdef CONFIG_MP_ION_PATCH_FAKE_MEM
	pr_info("%s %ld MiB at %pa\n\n", fakemem? "Not Reserved":"Reserved", (unsigned long)size / SZ_1M,
#else
                       ...
                       unsigned int align, long *retlen)
{
    ...
    pr_debug("%s:%d count=%#x, align=%#x, pageno=%#x, base_fn=%#lx, pfn=%#lx, page=%#x.\n", __func__, __LINE__,
        count, align, pageno, cma->base_pfn, pfn, page);
