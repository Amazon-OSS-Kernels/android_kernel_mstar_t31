	...
	unsigned int etdm_set)
{
	...
	mt8518_afe_configure_etdm_out(afe, etdm_set, rate,
		channels, bit_width);
	...
	unsigned int etdm_set)
{
	...
	mt8518_afe_configure_etdm_in(afe, etdm_set, rate,
		channels, bit_width);
...
static int mt8518_afe_handle_etdm_force_on(struct mtk_base_afe *afe)
{
	...
	if (!etdm1->force_on[SNDRV_PCM_STREAM_PLAYBACK] &&
	...
	if (etdm1->force_on[SNDRV_PCM_STREAM_PLAYBACK] ||
	    etdm1->clock_mode == MT8518_ETDM_SHARED_CLOCK) {
		...
		mt8518_afe_enable_etdm(afe, MT8518_ETDM1,
			SNDRV_PCM_STREAM_PLAYBACK);
	}

	if (etdm1->force_on[SNDRV_PCM_STREAM_CAPTURE] ||
	    etdm1->clock_mode == MT8518_ETDM_SHARED_CLOCK) {
	...
	if (etdm2->force_on[SNDRV_PCM_STREAM_PLAYBACK] ||
	    etdm2->clock_mode == MT8518_ETDM_SHARED_CLOCK) {
		...
		mt8518_afe_enable_etdm(afe, MT8518_ETDM2,
			SNDRV_PCM_STREAM_PLAYBACK);
	}

	if (etdm2->force_on[SNDRV_PCM_STREAM_CAPTURE] ||
	    etdm2->clock_mode == MT8518_ETDM_SHARED_CLOCK) {
...
static int mt8518_afe_dev_runtime_resume(struct device *dev)
{
	...
	mt8518_afe_handle_etdm_force_on(afe);
	...
	struct platform_device *pdev)
{
	...
	if (res) {
		afe_priv->afe_sram_va =
			devm_ioremap_resource(&pdev->dev, res);
		if (!IS_ERR(afe_priv->afe_sram_va)) {
			afe_priv->afe_sram_pa = res->start;
			afe_priv->afe_sram_size = resource_size(res);
		}
	}

	afe_priv->topckgen = syscon_regmap_lookup_by_phandle(np,
		"mediatek,topckgen-regmap");

	afe_priv->scpsys = syscon_regmap_lookup_by_phandle(np,
	...
	for (i = 0; i < ARRAY_SIZE(of_fe_table); i++) {
		...
		valid_sram = mt8518_afe_validate_sram(afe, temps[0], temps[1]);
		if (!valid_sram) {
...
static int mt8518_afe_pcm_probe(struct snd_soc_platform *platform)
{
	...
	ret = mt8518_afe_handle_etdm_force_on(afe);
	if (ret)
		return ret;
